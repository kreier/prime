...
if __name__ == "__main__":
    for i in range(len(scope)):
        ...
        for number in range(largest_divider + 2, last, 2):
            ...
            if (time.perf_counter() - dot) > 2:
                print(".", end="")
                dot = time.perf_counter()
                column += 1
                if column > 30:
                    t = elapsed_time((time.perf_counter() - start))
                    print(f" {t} - {number} {int(number*100/last)}% ")
                    column = 1
        end = time.perf_counter()
        if (end - start) > 2:
            print(" ")
        print(f"This took: {(end - start)} seconds. {elapsed_time((end - start)/1000000000)}")
        ...
        with open(filename, "w") as fp:
            ...
            fp.write(elapsed_time((end - start)))
